...
/*
7- Create a procedure Login that takes card number, PIN, and returns the status in an output
parameter. @status =1 if card number and PIN are valid and @status=0 otherwise.
(Hint: IF/ELSE).
Can we write UDF that works same as Login SP? Don’t write query, just explain in 1 to 2 lines*/
-- Create the UDF to get balance by card number

alter PROCEDURE GetUserCardCount
   @cardnum varchar(20), 
   @Ppin varchar(4), 
   @status int output
AS
...
END;
GO
-- Execute the stored procedure with a sample user ID
declare @r int;
EXEC GetUserCardCount '1234','170',@r output;
select @r as bal
GO